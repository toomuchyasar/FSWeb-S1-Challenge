...
function fenomenSil(array,index) {
  const newArray = array.slice(0);
...
function fenomenEkle(array,numara,profil,takipci,posta,site) {
  const newArray = array.slice(0);
  const newObj = {}
    newObj.number = numara,
    newObj.profile = profil,
    newObj.followers = takipci,
    newObj.posts = posta,
    newObj.platform = site
...
function enFenomenler(array) {
  ...
  for(let i = 0 ; i < newArray.length; i++){
      if(newArray[i].followers>100000000){
        listelenmisFenomenlerDizisi.push(newArray[i].profile)
        console.log(listelenmisFenomenlerDizisi);
      }
  }
  return listelenmisFenomenlerDizisi
}
...
function fenomenGonderimSayisi(array,profil){
  
  const yeniDiziolustur = [...array];
  for (let i = 0; i < yeniDiziolustur.length; i++) {
    if (yeniDiziolustur[i].profile.includes(profil)) {
...
function platformaGoreCokGonderiYapanFenomen(fenomenlerDizisi, platformAdi) {
  ...
  const maxFenomen = listelenmisFenomenlerDizisi.sort(function (
    ...
    return ikinciFenoObj.posts - ilkFenoObj.posts;
  });
  
  console.log("Görev 9", maxFenomen);
